...
class Asset(models.Model):
    ...
    business_unit = models.ForeignKey('BusinessUnit', null=True, blank=True, verbose_name='所属业务线',on_delete= True)
    status = models.SmallIntegerField(choices=asset_status, default=0, verbose_name='设备状态')

    manufacturer = models.ForeignKey('Manufacturer', null=True, blank=True, verbose_name='制造商',on_delete=True)
    manage_ip = models.GenericIPAddressField(null=True, blank=True, verbose_name='管理IP')
    tags = models.ManyToManyField('Tag', blank=True, verbose_name='标签')
    admin = models.ForeignKey(User, null=True, blank=True, verbose_name='资产管理员', related_name='admin',on_delete=True)
    idc = models.ForeignKey('IDC', null=True, blank=True, verbose_name='所在机房',on_delete=True)
    contract = models.ForeignKey('Contract', null=True, blank=True, verbose_name='合同',on_delete=True)
    ...
    approved_by = models.ForeignKey(User, null=True, blank=True, verbose_name='批准人', related_name='approved_by',on_delete=True)
...
class Server(models.Model):
    ...
    asset = models.OneToOneField('Asset',on_delete=True)  # 非常关键的一对一关联！
    ...
    hosted_on = models.ForeignKey('self', related_name='hosted_on_server',
                                  blank=True, null=True, verbose_name="宿主机",on_delete=True)  # 虚拟机专用字段
...
class SecurityDevice(models.Model):
    ...
    asset = models.OneToOneField('Asset',on_delete=True)
...
class BusinessUnit(models.Model):
    ...
    parent_unit = models.ForeignKey('self', blank=True, null=True, related_name='parent_level',on_delete=True)
...
class Contract(models.Model):
    ...
    sn = models.CharField('合同号', max_length=128, unique=True,help_text='合同编号',)
...
class CPU(models.Model):
    ...
    asset = models.OneToOneField('Asset',on_delete=True)  # 设备上的cpu肯定都是一样的，所以不需要建立多个cpu数据，一条就可以，因此使用一对一。
...
class RAM(models.Model):
    ...
    asset = models.ForeignKey('Asset',on_delete=True)  # 只能通过外键关联Asset。否则不能同时关联服务器、网络设备等等。
...
class Disk(models.Model):
    ...
    asset = models.ForeignKey('Asset',on_delete=True)
...
class NIC(models.Model):
    ...
    asset = models.ForeignKey('Asset',on_delete=True)  # 注意要用外键
